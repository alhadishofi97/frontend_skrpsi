...
const BarangList = () => {
  ...
  const [deleted, setOpen1] = useState(false);
  ...
  const columns = [
    { name: 'Part No', selector: (row) => row.part_no, sortable: true },
  ...
  const handleDelete = async () => {
    try {
      const response = await axios.delete(`https://backendapi.my.id/api/barang/delete_barang/${selectedRow.id_barang}`);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingItem) {
        ...
        const response = await axios.put(`https://backendapi.my.id/api/barang/update_barang/${editingItem.id_barang}`, formData, {
        ...
        // Create new item
        const barangResponse = await axios.post('https://backendapi.my.id/api/barang/create_barang', formData, {
          headers: { 'Content-Type': 'application/json' }
        });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const barangResponse = await axios.get('https://backendapi.my.id/api/barang/all_barang');
        if (barangResponse.data.status === 'success') {
        ...
        const categoriesResponse = await axios.get('https://backendapi.my.id/api/kategori-barang/all/kategori/barang');
        ...
        const typesMotorResponse = await axios.get('https://backendapi.my.id/api/type-motor/all_motor');
        if (typesMotorResponse.data.status === 'success') {
          setTypesMotor(typesMotorResponse.data.data);
        } else {
          setError('Failed to fetch types motor');
        }
      } catch (error) {
  ...
  return (
    <div>
      ...
      <Dialog open={deleted} onClose={handleClose}>
        ...
        <DialogContent>
          <DialogContentText>Are you sure you want to delete {selectedRow?.nama_barang}?</DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleClose} color="primary">
            Cancel
          </Button>
          <Button onClick={handleDelete} color="secondary" startIcon={<DeleteIcon />}>
            Delete
          </Button>
        </DialogActions>
      </Dialog>
